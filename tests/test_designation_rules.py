import pytest

from app.designations.rules import match

RULE_CASES: list[tuple[str | None, str, str | None]] = [
    ("CAT JHHMMSSss+DDMMSSs", "2MASSJ16295276+3911433", "2MASS J16295276+3911433"),
    ("CAT JHHMMSSss+DDMMSSs", "2massj16295276+3911433", "2MASS J16295276+3911433"),
    (None, "2MASSJ16295276+391143", None),
    (None, "2MASS J16295276+3911433", None),
    ("CAT JHHMMSSss+DDMMSSs", "2MASXJ21024892-2410520", "2MASX J21024892-2410520"),
    ("CAT JHHMMSSss+DDMMSSs", "2MASXJ09592150-2733330", "2MASX J09592150-2733330"),
    ("CAT JHHMMSSss+DDMMSSs", "2MASXJ10274703+2725368", "2MASX J10274703+2725368"),
    ("CAT JHHMMSSss+DDMMSSs", "2masxj09592150-2733330", "2MASX J09592150-2733330"),
    ("CAT HHMMSSs+DDMMSS", "SMDG1103517+284120", "SMDG 1103517+284120"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "SDSSJ121551.62+573421.6", "SDSS J121551.62+573421.6"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "SDSSJ121552.44+294932.9", "SDSS J121552.44+294932.9"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "SDSSJ121553.17+202452.4", "SDSS J121553.17+202452.4"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "sdssj121551.62+573421.6", "SDSS J121551.62+573421.6"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "SDSSJ121551.62-573421.6", "SDSS J121551.62-573421.6"),
    (None, "SDSSJ121551.62+57342.6", None),
    (None, "SDSS J121551.62+573421.6", None),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "GAMAJ121028.05+015906.4", "GAMA J121028.05+015906.4"),
    (None, "ZOAG241.08+02.22", None),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "WINGSJ125634.96-173656.6", "WINGS J125634.96-173656.6"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "WINGSJ232248.36+143927.6", "WINGS J232248.36+143927.6"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "WINGSJ234427.24+091831.8", "WINGS J234427.24+091831.8"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "WINGSJ005617.13-012206.3", "WINGS J005617.13-012206.3"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "WINGSJ132627.17-271414.2", "WINGS J132627.17-271414.2"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "wingsj125634.96-173656.6", "WINGS J125634.96-173656.6"),
    (None, "WINGS J125634.96-173656.6", None),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "2QZJ233027.0-280856", "2QZ J233027.0-280856"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "2QZJ121201.1+012648", "2QZ J121201.1+012648"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "2QZJ103110.2+001703", "2QZ J103110.2+001703"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "2qzj233027.0-280856", "2QZ J233027.0-280856"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "2QZJ233027-280856", "2QZ J233027-280856"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "LAMOSTJ003619.93+320329.5", "LAMOST J003619.93+320329.5"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "LAMOSTJ022723.16+013324.5", "LAMOST J022723.16+013324.5"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "lamostj085334.71+262244.1", "LAMOST J085334.71+262244.1"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "LAMOSTJ003619+320329", "LAMOST J003619+320329"),
    ("LV", "LVJ1235-1104", "LV J1235-1104"),
    ("LV", "LVJ1052+3628", "LV J1052+3628"),
    ("LV", "lvj1235-1104", "LV J1235-1104"),
    ("CAT J HHMM+DD", "HIPASSJ1558-10", "HIPASS J1558-10"),
    ("CAT J HHMM+DD", "HIPASS J1348-37", "HIPASS J1348-37"),
    ("CAT J HHMM+DD", "HIPASS J0916-23b", "HIPASS J0916-23b"),
    ("CAT J HHMM+DD", "hipassj1558-10", "HIPASS J1558-10"),
    (None, "HIPASSJ1558-1", None),
    ("CAT J HHMM+DD", "HIPASS 1348-37", "HIPASS 1348-37"),
    ("CAT J HHMM+DD", "SMDG1256-00", "SMDG 1256-00"),
    ("CAT J HHMM+DD", "SMDG0740+40", "SMDG 0740+40"),
    ("CAT J HHMM+DD", "smdg1349+04", "SMDG 1349+04"),
    ("CAT J HHMM+DD", "SMDG 1256-00", "SMDG 1256-00"),
    (None, "SMDG1256-0", None),
    ("CAT J HHMM+DD", "KUG 0937+480", "KUG 0937+480"),
    ("CAT J HHMM+DD", "KUG0937+48", "KUG 0937+48"),
    ("CAT J HHMM+DD", "kug0937+480", "KUG 0937+480"),
    (None, "KUG0937+4", None),
    ("CAT J HHMM+DD", "SBS 1224+533", "SBS 1224+533"),
    ("CAT J HHMM+DD", "SBS1224+53", "SBS 1224+53"),
    ("CAT J HHMM+DD", "sbs1224+533", "SBS 1224+533"),
    (None, "SBS1224+5", None),
    ("CAT J HHMM+DD", "FASHI1334+62", "FASHI 1334+62"),
    ("CAT J HHMM+DD", "FASHI0252+40", "FASHI 0252+40"),
    ("CAT J HHMM+DD", "FASHI 1251+57", "FASHI 1251+57"),
    ("CAT J HHMM+DD", "fashi0237+38", "FASHI 0237+38"),
    (None, "FASHI1334+6", None),
    ("CAT HHMM+DDMM", "MAGE1423+5613", "MAGE 1423+5613"),
    ("CAT HHMM+DDMM", "MAGE0740+1651", "MAGE 0740+1651"),
    ("CAT HHMM+DDMM", "mage1132+5311", "MAGE 1132+5311"),
    ("CAT HHMM+DDMM", "MAGE 1423+5613", "MAGE 1423+5613"),
    ("CAT J HHMM+DD", "MAGE1423+561", "MAGE 1423+561"),
    ("[REF]J", "[BZZ2000]J132523.95-305406.7", "[BZZ2000] J132523.95-305406.7"),
    ("[REF]J", "[BZZ2000] J132523.95-305406.7", "[BZZ2000] J132523.95-305406.7"),
    ("[REF]J", "[IBG2003]J114222+195346", "[IBG2003] J114222+195346"),
    ("[REF]J", "[RGD2016]J130127.14+275957.1", "[RGD2016] J130127.14+275957.1"),
    ("[REF]J", "[EKS96]J123456+123456", "[EKS96] J123456+123456"),
    (None, "[123]J132523.95-305406.7", None),
    (None, "[ABC]J132523.95-305406.7", None),
    ("CAT NNNNNN", "NGC 123", "NGC 123"),
    ("CAT NNNNNN", "NGC 905", "NGC 905"),
    ("CAT NNNNNN", "NGC905", "NGC 905"),
    ("CAT NNNNNN", "NGC00905", "NGC 905"),
    ("CAT NNNNNN", "NGC  905", "NGC 905"),
    ("CAT NNNNNN", "ngc 905", "NGC 905"),
    ("CAT NNNNNN", "NGC1487a", "NGC 1487a"),
    ("CAT NNNNNN", "NGC1487b", "NGC 1487b"),
    ("CAT NNNNNN", "NGC3521sat", "NGC 3521sat"),
    (None, "NGC", None),
    (None, "NGC ", None),
    ("M", "M 31", "M 31"),
    ("CAT NNNNNN", "UGC 123", "UGC 123"),
    ("CAT NNNNNN", "PGC1191069", "PGC 1191069"),
    ("CAT NNNNNN", "PGC1119121", "PGC 1119121"),
    ("CAT NNNNNN", "PGC1425552", "PGC 1425552"),
    ("CAT NNNNNN", "PGC 1191069", "PGC 1191069"),
    ("CAT NNNNNN", "PGC001191069", "PGC 1191069"),
    ("CAT NNNNNN", "pgc1191069", "PGC 1191069"),
    (None, "PGC", None),
    (None, "PGC ", None),
    ("CAT NNNNNN", "GAMA077963", "GAMA 77963"),
    ("CAT NNNNNN", "GAMA028694", "GAMA 28694"),
    ("CAT NNNNNN", "GAMA297074", "GAMA 297074"),
    ("CAT NNNNNN", "GAMA345748", "GAMA 345748"),
    ("CAT NNNNNN", "GAMA 77963", "GAMA 77963"),
    ("CAT NNNNNN", "gama077963", "GAMA 77963"),
    (None, "GAMA", None),
    ("CAT NNNNNN", "MGC56790", "MGC 56790"),
    ("CAT NNNNNN", "MGC47128", "MGC 47128"),
    ("CAT NNNNNN", "MGC 56790", "MGC 56790"),
    ("CAT NNNNNN", "mgc56790", "MGC 56790"),
    (None, "MGC", None),
    ("CAT NNNNNN", "HIP113070", "HIP 113070"),
    ("CAT NNNNNN", "HIP 113070", "HIP 113070"),
    ("CAT NNNNNN", "hip113070", "HIP 113070"),
    (None, "HIP", None),
    ("CAT NNNNNN", "HD097243", "HD 97243"),
    ("CAT NNNNNN", "HD044063", "HD 44063"),
    ("CAT NNNNNN", "HD073216", "HD 73216"),
    ("CAT NNNNNN", "HD158365", "HD 158365"),
    ("CAT NNNNNN", "HD114550", "HD 114550"),
    ("CAT NNNNNN", "HD267907", "HD 267907"),
    ("CAT NNNNNN", "HD073612", "HD 73612"),
    ("CAT NNNNNN", "HD 97243", "HD 97243"),
    ("CAT NNNNNN", "hd097243", "HD 97243"),
    (None, "HD", None),
    ("CAT NNNNNN", "VCC0114", "VCC 114"),
    ("CAT NNNNNN", "VCC 114", "VCC 114"),
    ("CAT NNNNNN", "vcc0114", "VCC 114"),
    (None, "VCC", None),
    ("CAT NNNNNN", "EVCC2232", "EVCC 2232"),
    ("CAT NNNNNN", "EVCC 2232", "EVCC 2232"),
    ("CAT NNNNNN", "evcc2232", "EVCC 2232"),
    (None, "EVCC", None),
    ("CAT NNNNNN", "UCD330", "UCD 330"),
    ("CAT NNNNNN", "UCD 330", "UCD 330"),
    ("CAT NNNNNN", "ucd330", "UCD 330"),
    (None, "UCD", None),
    ("CAT NNNNNN", "UGCA442", "UGCA 442"),
    ("CAT NNNNNN", "UGCA 442", "UGCA 442"),
    ("CAT NNNNNN", "ugca442", "UGCA 442"),
    ("CAT NNNNNN", "UGC 442", "UGC 442"),
    ("CAT NNNNNN", "KKS74", "KKS 74"),
    ("CAT NNNNNN", "KKS 74", "KKS 74"),
    ("CAT NNNNNN", "kks74", "KKS 74"),
    ("CAT NNNNNN", "KKSG37", "KKSG 37"),
    ("CAT NNNNNN", "UGC05983", "UGC 5983"),
    ("CAT NNNNNN", "UGC07085W", "UGC 7085W"),
    ("CAT NNNNNN", "UGC02140A", "UGC 2140A"),
    ("CAT NNNNNN", "UGC 5983", "UGC 5983"),
    ("CAT NNNNNN", "ugc07020a", "UGC 7020a"),
    (None, "UGC", None),
    ("CAT NNNNNN", "IC1613", "IC 1613"),
    ("CAT NNNNNN", "IC 1613", "IC 1613"),
    ("CAT NNNNNN", "IC1670A", "IC 1670A"),
    ("CAT NNNNNN", "ic1670a", "IC 1670a"),
    ("CAT NNNNNN", "ic1613", "IC 1613"),
    (None, "IC", None),
    ("CAT NNNNNN", "DDO217", "DDO 217"),
    ("CAT NNNNNN", "DDO226", "DDO 226"),
    ("CAT NNNNNN", "DDO 217", "DDO 217"),
    ("CAT NNNNNN", "ddo217", "DDO 217"),
    (None, "DDO", None),
    ("CAT NNNNNN", "AGC724906", "AGC 724906"),
    ("CAT NNNNNN", "AGC731457", "AGC 731457"),
    ("CAT NNNNNN", "AGC 724906", "AGC 724906"),
    ("CAT NNNNNN", "agc724906", "AGC 724906"),
    (None, "AGC", None),
    ("CAT NNNNNN", "KDG056", "KDG 56"),
    ("CAT NNNNNN", "KDG162", "KDG 162"),
    ("CAT NNNNNN", "KDG 216", "KDG 216"),
    ("CAT NNNNNN", "kdg056", "KDG 56"),
    (None, "KDG", None),
    ("CAT NNNNNN", "KKSG37", "KKSG 37"),
    ("CAT NNNNNN", "KKSG34", "KKSG 34"),
    ("CAT NNNNNN", "kksg37", "KKSG 37"),
    (None, "KKSG", None),
    ("CAT NNNNNN", "KKH69", "KKH 69"),
    ("CAT NNNNNN", "KKH 69", "KKH 69"),
    ("CAT NNNNNN", "kkh69", "KKH 69"),
    ("CAT NNNNNN", "KK69", "KK 69"),
    (None, "KKH", None),
    ("CAT NNNNNN", "KKR18", "KKR 18"),
    ("CAT NNNNNN", "KKR 18", "KKR 18"),
    ("CAT NNNNNN", "kkr18", "KKR 18"),
    (None, "KKR", None),
    ("CAT NNNNNN", "KK203", "KK 203"),
    ("CAT NNNNNN", "KK242", "KK 242"),
    ("CAT NNNNNN", "KK 203", "KK 203"),
    ("CAT NNNNNN", "kk242", "KK 242"),
    ("CAT NNNNNN", "KKH69", "KKH 69"),
    ("CAT NNNNNN", "KKR18", "KKR 18"),
    (None, "KK", None),
    ("CAT NNNNNN", "BTS132", "BTS 132"),
    ("CAT NNNNNN", "BTS157", "BTS 157"),
    ("CAT NNNNNN", "BTS 132", "BTS 132"),
    ("CAT NNNNNN", "bts132", "BTS 132"),
    (None, "BTS", None),
    ("CAT NNNNNN", "JKB129", "JKB 129"),
    ("CAT NNNNNN", "JKB 129", "JKB 129"),
    ("CAT NNNNNN", "jkb129", "JKB 129"),
    (None, "JKB", None),
    ("CAT NNNNNN", "PANDAS57", "PANDAS 57"),
    ("CAT NNNNNN", "PANDAS 57", "PANDAS 57"),
    ("CAT NNNNNN", "pandas57", "PANDAS 57"),
    (None, "PANDAS", None),
    ("CAT NNNNNN", "LAEVENS2", "LAEVENS 2"),
    ("CAT NNNNNN", "LAEVENS 2", "LAEVENS 2"),
    ("CAT NNNNNN", "laevens2", "LAEVENS 2"),
    (None, "LAEVENS", None),
    ("CAT NNNNNN", "BSDL2844", "BSDL 2844"),
    ("CAT NNNNNN", "BSDL 2844", "BSDL 2844"),
    ("CAT NNNNNN", "bsdl2844", "BSDL 2844"),
    (None, "BSDL", None),
    ("CAT +HHMMSS", "BD+062136", "BD +062136"),
    ("CAT +HHMMSS", "BD+391991", "BD +391991"),
    ("CAT +HHMMSS", "BD-123456", "BD -123456"),
    ("CAT +HHMMSS", "BD +062136", "BD +062136"),
    ("CAT +HHMMSS", "bd+062136", "BD +062136"),
    ("CAT NNNNNN", "BD062136", "BD 62136"),
    ("NGC", "N3902", "NGC 3902"),
    ("NGC", "N7448", "NGC 7448"),
    ("NGC", "n3902", "NGC 3902"),
    ("NGC", "N 7448", "NGC 7448"),
    ("3C", "3C223.1", "3C 223.1"),
    ("3C", "3C192", "3C 192"),
    ("3C", "3C 236", "3C 236"),
    ("3C", "3c310", "3C 310"),
    ("3C", "3C218A", "3C 218A"),
    ("3C", "3C 218A", "3C 218A"),
    ("3C", "3c218a", "3C 218a"),
    (None, "3C", None),
    ("2dFGRS", "2DFGRSN129Z061", "2dFGRS N129Z061"),
    ("2dFGRS", "2dfgrsN129Z061", "2dFGRS N129Z061"),
    ("2dFGRS", "2DFGRSS889Z059", "2dFGRS S889Z059"),
    ("2dFGRS", "2dFGRS N129Z061", "2dFGRS N129Z061"),
    ("J", "J115524.38+391310.9", "J115524.38+391310.9"),
    ("J", "J091854.30-060439.5", "J091854.30-060439.5"),
    ("J", "j044444.77+320646.6", "J044444.77+320646.6"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "SDSSJ121551.62+573421.6", "SDSS J121551.62+573421.6"),
    ("M", "MESSIER082", "M 82"),
    ("M", "M82", "M 82"),
    ("M", "M 82", "M 82"),
    ("M", "m31", "M 31"),
    ("CAT NNNNNN", "MCG 123", "MCG 123"),
    ("Andromeda", "And XXVIII", "Andromeda 28"),
    ("Andromeda", "Andromeda XXVIII", "Andromeda 28"),
    ("Andromeda", "And28", "Andromeda 28"),
    ("Andromeda", "ANDXXVIII", "Andromeda 28"),
    ("Andromeda", "andxxviii", "Andromeda 28"),
    ("Andromeda", "andromeda28", "Andromeda 28"),
    ("Andromeda", "And 28", "Andromeda 28"),
    ("Andromeda", "Andromeda I", "Andromeda 1"),
    ("Andromeda", "Andromeda IV", "Andromeda 4"),
    ("Andromeda", "And X", "Andromeda 10"),
    (None, "Andromeda", None),
    (None, "And", None),
    ("ESO", "ESO104-044", "ESO 104-44"),
    ("ESO", "ESO381-020", "ESO 381-20"),
    ("ESO", "ESO 104-044", "ESO 104-44"),
    ("ESO", "eso104-044", "ESO 104-44"),
    ("ESO", "ESO188-012A", "ESO 188-12A"),
    ("ESO", "ESO410-G005", "ESO 410-G5"),
    ("ESO", "ESO112-008A", "ESO 112-8A"),
    ("ESO", "eso146-006a", "ESO 146-6a"),
    ("CAT NNNNNN", "ESO104", "ESO 104"),
    (None, "ESO", None),
    ("CGCG", "CGCG 066-109", "CGCG 066-109"),
    ("CGCG", "CGCG217-018", "CGCG 217-18"),
    ("CGCG", "cgcg066-109", "CGCG 066-109"),
    ("CGCG", "CGCG 066-09", "CGCG 066-09"),
    (None, "CGCG66-109", None),
    ("DR8", "DR8-1996p672-237", "DR8-1996p672-237"),
    ("DR8", "DR8-3423p297-8", "DR8-3423p297-8"),
    ("DR8", "DR8-2056p285-12718", "DR8-2056p285-12718"),
    ("DR8", "DR8-0819m622-13740", "DR8-0819m622-13740"),
    ("DR8", "DR8-0850m475-585", "DR8-0850m475-585"),
    ("DR8", "dr8-1236p155-1835", "DR8-1236p155-1835"),
    (None, "DR8-1996p67-237", None),
    ("ISI96", "ISI96_1450+0230", "ISI96_1450+0230"),
    ("ISI96", "ISI96_2339-0122", "ISI96_2339-0122"),
    ("ISI96", "ISI96_1042+0231", "ISI96_1042+0231"),
    ("ISI96", "isi96_0922+0233", "ISI96_0922+0233"),
    (None, "ISI96_1450+023", None),
    ("AM", "AM 0316-484", "AM 0316-484"),
    ("AM", "AM2220-460", "AM 2220-460"),
    ("AM", "am0316-484", "AM 0316-484"),
    ("AM", "AM 2220-460", "AM 2220-460"),
    (None, "AM0316-4", None),
    ("Dw", "Dw1311+4051", "Dw 1311+4051"),
    ("Dw", "Dw1106-0052", "Dw 1106-0052"),
    ("Dw", "dw1048+1259", "Dw 1048+1259"),
    ("Dw", "dw1323-40", "Dw 1323-40"),
    ("Dw", "dw1051+11", "Dw 1051+11"),
    ("Dw", "dw1045+14b", "Dw 1045+14b"),
    ("Dw", "Dw1045+14B", "Dw 1045+14b"),
    ("Dw", "Dw1252-0506", "Dw 1252-0506"),
    ("Dw", "Dw 1311+4051", "Dw 1311+4051"),
    ("CAT J HHMM+DD", "Dw1311+405", "DW 1311+405"),
    (None, "Dw1311+40511", None),
    (None, "Dw", None),
    ("KSP-DW", "KSP-DW28", "KSP-DW 28"),
    ("KSP-DW", "KSP-DW1", "KSP-DW 1"),
    ("KSP-DW", "KSP-DW 12", "KSP-DW 12"),
    ("KSP-DW", "ksp-dw30", "KSP-DW 30"),
    (None, "KSP-DW", None),
    ("LSBC", "LSBC D640-12", "LSBC D640-12"),
    ("LSBC", "LSBC D565-06", "LSBC D565-6"),
    ("LSBC", "lsbc d640-12", "LSBC D640-12"),
    (None, "LSBC D640", None),
    ("CAT HHMM+DDMM", "LV 1235-1104", "LV 1235-1104"),
    ("MCG", "MCG -04-31-038", "MCG -04-31-38"),
    ("MCG", "MCG -04-31-38", "MCG -04-31-38"),
    ("MCG", "mcg -04-31-038", "MCG -04-31-38"),
    ("MCG", "MCG 04-31-038", "MCG 04-31-38"),
    (None, "MCG", None),
    ("6dF", "6dFJ2032543-251656", "6dF J2032543-251656"),
    ("6dF", "6dFJ1605369-301035", "6dF J1605369-301035"),
    ("6dF", "6dFJ1253111-272754:", "6dF J1253111-272754"),
    ("6dF", "6dfj2016142-085824", "6dF J2016142-085824"),
    ("CAT JHHMMSS.sss+DDMMSS.sss", "6dFJ203254-251656", "6DF J203254-251656"),
    ("ABELL", "ABELL0076", "ABELL 76"),
    ("ABELL", "ABELL0496:[DAL2002]11866", "ABELL 496 [DAL2002] 11866"),
    ("ABELL", "ABELL2204_14:[PSE2006]2096", "ABELL 2204_14 [PSE2006] 2096"),
    ("ABELL", "ABELL2744:[BGS2006]011", "ABELL 2744 [BGS2006] 011"),
    ("ABELL", "abell0496:[DAL2002]11866", "ABELL 496 [DAL2002] 11866"),
    ("ABELL", "ABELL0496", "ABELL 496"),
    ("ABELL", "ABELL 496", "ABELL 496"),
    ("CNOC2", "CNOC2_0223.051394", "CNOC2_0223.051394"),
    ("CNOC2", "CNOC2_0223.100322", "CNOC2_0223.100322"),
    ("CNOC2", "cnoc2_0223.140942", "CNOC2_0223.140942"),
    ("CNOC2", "CNOC2_0223.090502", "CNOC2_0223.090502"),
    (None, "CNOC20223.051394", None),
    ("2MFGC", "2MFGC15266", "2MFGC 15266"),
    ("2MFGC", "2MFGC 15266", "2MFGC 15266"),
    ("2MFGC", "2mfgc15266", "2MFGC 15266"),
    (None, "2MFGC", None),
    ("VVDS", "VVDS220456214", "VVDS 220456214"),
    ("VVDS", "VVDS 220456214", "VVDS 220456214"),
    ("VVDS", "vvds220456214", "VVDS 220456214"),
    (None, "VVDS", None),
    ("GALFA", "GALFAJ180.8+24.1+032", "GALFA J180.8+24.1+032"),
    ("GALFA", "galfaj180.8+24.1+032", "GALFA J180.8+24.1+032"),
    (None, "GALFAJ180.8+24.1", None),
    ("USGC", "USGCU169", "USGC U169"),
    ("USGC", "usgcu169", "USGC U169"),
    (None, "USGC U169", None),
    ("RXJ", "RXJ0152.7-1357", "RX J0152.7-1357"),
    ("RXJ", "RXJ0152.7-1357:[DRL2005]0396", "RX J0152.7-1357 [DRL2005] 0396"),
    ("RXJ", "rxj0152.7-1357", "RX J0152.7-1357"),
    ("RXJ", "RXJ0152-1357", "RX J0152-1357"),
    ("CLJ", "CLJ1604+4304", "CL J1604+4304"),
    ("CLJ", "CLJ1604+4304:[PLO98]3268", "CL J1604+4304 [PLO98] 3268"),
    ("CLJ", "clj1604+4304", "CL J1604+4304"),
    (None, "CLJ160+4304", None),
    ("SMMJ", "SMMJ221733.02+000906.0", "SMM J221733.02+000906.0"),
    ("SMMJ", "smmj221733.02+000906.0", "SMM J221733.02+000906.0"),
    ("SMMJ", "SMMJ221733+000906", "SMM J221733+000906"),
    ("NGC", "NGC1672:[EKS96]207", "NGC 1672 [EKS96] 207"),
    ("NGC", "NGC 1672:[EKS96]207", "NGC 1672 [EKS96] 207"),
    ("NGC", "ngc1672:[EKS96]207", "NGC 1672 [EKS96] 207"),
    (None, "", None),
]


@pytest.mark.parametrize("rule_name,input_name,expected", RULE_CASES)
def test_rule_match(rule_name: str | None, input_name: str, expected: str | None) -> None:
    result = match(input_name)
    if expected is None:
        assert result is None
    else:
        assert result is not None
        formatted, matched_rule_name = result
        assert formatted == expected
        assert matched_rule_name == rule_name


def test_unmatched_name_returns_none() -> None:
    assert match("unknown catalog XYZ 123") is None
